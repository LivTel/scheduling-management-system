 ...
 * 
 */
public class DefaultExecutionUpdater extends UnicastRemoteObject implements ExecutionUpdater {

	//private IPhase2Model phase2Model;

	/**
	 * A synoptic model provider.
	 */
	...
	/**
	 * ID of the execution history instance with which this updater is
	 * associated.
	 */
	long xid;

	/**
	 * Create a DefaultExecutionUpdater.
	 * 
	 * @throws RemoteException
	 */
	public DefaultExecutionUpdater(SynopticModelProvider smp) throws RemoteException {
		super();

		this.smp = smp;
		// TODO Some of the things it will need to perform.
		//
		// xid: The execution ID.
		// sequence: The actual sequence (for calculating cost etc)
		// Ident: various IDs and names of TAG,User,Proposal, Group etc
		// Group: All the relevant group information - constraints, prefs.
		//
		// XCost: Any additional costs other than those calculated as standard.
		// e.g. bidding amounts, prime-condition rates
	...
	/**
	 * Set the charging model to use.
	 * 
	 * @param chargeModel
	 *            the chargeModel to set
	 */
	public void setChargeModel(ChargeAccountingModel chargeModel) {
	...
	public void groupExecutionAbandoned(GroupItem group, long time, ExecutionResourceBundle erb,
		...
		// historyModel.updateHistory(group.getHId(),
		// IHistoryItem.EXECUTION_FAILED, time, efc, qosMetrics);
		hsm.updateHistory(group.getID(), group.getHId(), IHistoryItem.EXECUTION_FAILED, time, efc, qosMetrics);
	...
	public void groupExecutionCompleted(GroupItem group, long time, ExecutionResourceBundle erb,
		...
		hsm.updateHistory(group.getID(), group.getHId(), IHistoryItem.EXECUTION_SUCCESSFUL, time, null, qosMetrics);

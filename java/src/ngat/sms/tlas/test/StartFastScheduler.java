 ...
 * 
 */
public class StartFastScheduler {
	...
	public static void main(String[] args) {
		...
		LogGenerator logger = alogger.generate().system("SMS").subSystem("SchedulingStatusProvider")
		...
		while (!bound) {
			...
			try {
				...
				// Phase2GroupModelProvider p2g = (Phase2GroupModelProvider)
				// Naming.lookup(p2gUrl);
				 ...
				 * 
				 * // Create Synoptic model BasicHistorySynopsisModel bsm = new
				 * BasicHistorySynopsisModel(history);
				 * collator.msg("Created Synoptic History Model").send();
				 * 
				 * // Accounting models
				 * 
				 * // Proposals String accUrl = "rmi://" + bhost + ":" + port +
				 * "/ProposalAccountModel"; IAccountModel accModel =
				 * (IAccountModel) Naming.lookup(accUrl);
				 * collator.msg("Located Proposal Accounting Model using: " +
				 * accUrl).send();
				 * 
				 * // Proposal Acc synopsis. BasicAccountSynopsisModel pasm =
				 * new BasicAccountSynopsisModel(accModel);
				 * collator.msg("Created Synoptic Proposal Account Model"
				 * ).send();
				 * 
				 * // TAGs String taccUrl = "rmi://" + bhost + ":" + port +
				 * "/TagAccountModel"; IAccountModel taccModel = (IAccountModel)
				 * Naming.lookup(taccUrl);
				 * collator.msg("Located TAG Accounting Model using: " +
				 * taccUrl).send();
				 * 
				 * // TAG Acc synopsis. BasicAccountSynopsisModel tasm = new
				 * BasicAccountSynopsisModel(taccModel);
				 * collator.msg("Created Synoptic Proposal Account Model"
				 * ).send();
				 * 
				 * 
				...
				// scope.setDomeLimit(Math.toRadians(config.getDoubleValue("dome.limit",
				// 25.0)));
				try {
				...
				StandardExecutionFeasibilityModel bxm = new StandardExecutionFeasibilityModel(calc, xrm, chargeModel,
						site, scope, bit);
				...
				FastLookAheadScheduler flas = new FastLookAheadScheduler(timeModel, site, bxm, xrm, bit, scope, smp,
						astro, xm);

				flas.setSkyModelUrl(smUrl);
				flas.setHorizon(3600 * 1000L);
				flas.setNumberSweeps(100);
				collator.msg("Created SchedulingStatusProvider").send();

				String schedUrl = "rmi://" + host + ":" + port + "/SchedulingStatusProvider";
				Naming.rebind(schedUrl, flas);
				...
				Naming.rebind(sdUrl, flas);
				logger.create().extractCallInfo().info().level(1).msg("Bound SchedulingStatusProvider as: " + sdUrl)
						.send();

				String asdUrl = "rmi://" + host + ":" + port + "/AsynchScheduleDespatcher";
				Naming.rebind(asdUrl, flas);
				logger.create().extractCallInfo().info().level(1).msg("Bound SchedulingStatusProvider as: " + asdUrl)
						.send();
				...
				VetoManager vetoManager = (VetoManager) flas;
				// this is slightly risky, really we should pass a seperate VC
				// in here rather than just
				// assume TSD is "one-of-them"
				logger.create().extractCallInfo().info().level(1).msg("Starting legacy server...").send();

				VetoManager vmgr = (VetoManager)flas;
				ScheduleServer server = new ScheduleServer("SMS_SERVER", smsPort, flas, vmgr, gmap);
				server.start();
				...
				logger.create().extractCallInfo().info().level(1).msg("Staring asynch prescan...").send();

				flas.asynchRunPrescan();
